...
@pytest.mark.parametrize('log_all_data', [True, False])
@pytest.mark.gpu
@pytest.mark.world_size(2)
def test_global_straggler_detector_all(device, flops_per_batch: bool, log_all_data: bool):
    dist.initialize_dist(get_device(device))

    # Construct the callbacks
    global_straggler_detector = GlobalStragglerDetector(log_all_data=log_all_data)
    ...
    if log_all_data:
        ...
        if dist.get_local_rank() == 0:
            for rank in range(0, dist.get_world_size()):
                _assert_no_negative_values(in_memory_logger.data['RoundTripTime/Rank-' + str(rank)], "ms")
                _assert_no_negative_values(in_memory_logger.data['Power/Rank-' + str(rank)], "W")
                _assert_no_negative_values(in_memory_logger.data['Temp/Rank-' + str(rank)], "C")
                _assert_no_negative_values(in_memory_logger.data['Utilization/Rank-' + str(rank)], "%")
                _assert_leq_hundred(in_memory_logger.data['Utilization/Rank-' + str(rank)], "%")
                _assert_no_negative_values(in_memory_logger.data['Clock/Rank-' + str(rank)], "MHz")
                ...
                assert len(in_memory_logger.data['Utilization/Rank-' + str(rank)]) == num_batches
                assert len(in_memory_logger.data['Clock/Rank-' + str(rank)]) == num_batches
                assert len(in_memory_logger.data['BatchLoadLatency/Rank-' + str(rank)]) == num_batches
